...
<!-- 페이징 처리부분 추가된 freeboard_list.jsp -->
...
<!-- JSP 코드 블락 : DB의 레코드를 가져와서 루프를 돌린다. : 시작 -->
	<%
	
	/* DB에서 값을 가져와서 Vector 컬렉션에 저장 후 페이징 처리 */
	
	//Vector 변수 선언 : 0번부터 시작
	...
	//go 변수를 넘겨받아서 wheregroup, startpage, endpage 정보를 알아낼 수 있다.	[1][2][3][4][5][6]...
		//코드 블락
	...
	String em = null;	//DB에서 mail 주소를 가져와서 처리하는 변수
		...
		}else{	// rs의 값이 존재할때(게시판의 글이 존재할떄)
			do{
				
			// rs에 담긴 ""의 값이 vector에 담긴다.
		...
		// 페이징 변수 출력
		// startrow와 endrow는 index0 부터 시작한다.
		/*
		out.println("<p/> 총 레코드 개수 (totalrows : ) : " + totalrows + "<p/>");
		out.println("<p/> 전체 페이지 수 (totalpages : ) : " + totalpages + "<p/>");
		out.println("<p/> 총 레코드 개수 (startrow : ) : " + startrow + "<p/>");
		out.println("<p/> 총 레코드 개수 (endrow : ) : " + endrow + "<p/>");
		*/
...
<!-- JSP 코드 블락 : DB의 레코드를 가져와서 루프를 돌린다. : 끝 -->
 </TABLE>
