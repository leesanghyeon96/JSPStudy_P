...
<% 
      ...
      if (what == 1) {
    	  out.println ("<OPTION value=1 selected >이름 </OPTION>"); 	// select : key값
...
<TABLE border=0 width=600 cellpadding=4 cellspacing=0>
 ...
 int id=0;
 	...
 	String sql = null; 
 	Statement stmt = null; 	// 쿼리를 담아 DB에 보내는 객체
 	ResultSet rs = null; 	// select한 쿼리를 리턴을 담아 저장하는 객체
 	
 	//cond : option 에 선택자에 따라서 다른 쿼리가 변수로 들어옴. 
 	sql = "select * from freeboard " + cond ;	// cond가 where로 시작되어 공백이 들어가야한다.
 	sql += " order by id desc" ; 
 
 	
 	 //out.println (sql);		// prepareStatement로 출력하면 출력이 안됨(해도되긴함)
 	...
 	rs = stmt.executeQuery(sql); 	// select문이기 떄문에 executeQuery를 사용
 	
 	if (!(rs.next())){
 		out.println("해당내용이 DB 에 존재 하지 않습니다. ");
 	} else {   //검색된 내용이 존재 한다면 
 		
 		do {	// rs.next()의 값이 존재하지 않을때까지 레코드의 값을 저장(Vector에)
 				// 페이징 처리하기위해 (만약 하지않으면 그냥 rs.getString()으로 찍으면 된다.)
